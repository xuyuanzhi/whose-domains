...
| Observation boundary | Migration leaves `WEB_RETENTION_FACT_COLLECTION.COLLECTION_STARTED_ON` null. The first successful writer transaction activates it; `WEB_RETENTION_FACT_HEALTH` records non-user-level daily successes, failures, and expected row counts. Reports require a continuous, gap-free 120-day interval and reject interrupted or prematurely cleaned history. |
