...
| Observation boundary | Migration leaves `WEB_RETENTION_FACT_COLLECTION.COLLECTION_STARTED_ON` null. The writer records only non-user-level observed counts/heartbeats in `WEB_RETENTION_FACT_HEALTH`; rows remain `OPEN` and cannot prove their own completeness. After independent operational reconciliation, run `scripts/verify-retention-fact-day.sql` for each concrete date. Reports require 120 continuous `VERIFIED` days and reject `OPEN`, missing, interrupted, or prematurely cleaned history. Writer, verifier, and report use `WESITE_RETENTION_REPORTING_ZONE` (default `Asia/Shanghai`; SQL offset `+08:00`). |
