...
CREATE TABLE `WEB_DOMAIN_DNS` (
  ...
  `CITY_JSON`   text,
  `ASN_JSON`    text,
  `DNS_LIVE_FINGERPRINT` binary(32) GENERATED ALWAYS AS (
    CASE WHEN `DELETED` = 0 THEN
      UNHEX(SHA2(CONCAT(HEX(`DOMAIN_ID`), ':', HEX(`NAME`), ':', HEX(`TYPE`), ':', HEX(`VALUE`)), 256))
    ELSE NULL END
  ) VIRTUAL,
  PRIMARY KEY (`ID`),
  UNIQUE KEY `UK_DNS_LIVE_FINGERPRINT` (`DNS_LIVE_FINGERPRINT`),
